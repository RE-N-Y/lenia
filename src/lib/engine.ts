...
export const lenia = (kh:number, kw:number, bumps:number, channels:number, speed:number=.1, epsilon:number=4e-8 ) => {
    ...
    // tensor to image
    const convert = (x:tf.Tensor4D) => tf.tidy(() => x.mean([-1,0]).clipByValue(0,1))
    ...
    const constructKernel = (alphas:tf.Tensor, betas:tf.Tensor, weights:tf.Tensor) => {
        ...
        const result = tf.tidy(() => {
            ...
            const r = radius().reshape([kh, kw, 1, 1]).tile([1,1,bumps,channels])
